...
from deal_engine import (
    extract_product, generate_caption, tag_affiliate_url,
    detect_platform, send_telegram, test_telegram_connection
)
...
async def deal_engine_extract(request: Request):
    ...
    logger.info(f"[Deal Engine] Extract request for URL: {url[:80]}")
    api_key = os.environ.get("EMERGENT_LLM_KEY")
    ...
    logger.info(f"[Deal Engine] Extraction complete: platform={platform}, title={product.get('title','')[:50]}")
...
async def deal_engine_settings(request: Request):
    ...
    await admin_required(request)
    logger.info("[Deal Engine] Settings GET request")
    settings = await db.site_settings.find_one({"_id": "deal_engine"}) or {}
    settings.pop("_id", None)
...
async def update_deal_engine_settings(request: Request):
    ...
    body = await request.json()
    body.pop("_id", None)
    logger.info("[Deal Engine] Settings PATCH request")
    await db.site_settings.update_one(
        {"_id": "deal_engine"},
        {"$set": body},
        upsert=True
...
async def startup():
    logger.info("==> Disccart API starting up...")
    ...
    logger.info("==> Disccart API startup complete. All routes registered.")
    logger.info("==> Deal Engine routes: /api/deal-engine/extract, /api/deal-engine/settings, /api/deal-engine/queue")
