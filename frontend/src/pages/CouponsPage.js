          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-bold text-[#FF8C00] uppercase tracking-wide">{coupon.brand_name}</span>
            ...
            <div className="ml-3 flex-shrink-0 bg-[#228B22] text-white font-black text-sm px-3 py-1.5 rounded-xl font-display">
              {getDiscountDisplay()}
            </div>
          </div>
        ...
        <div className="border-t border-dashed border-gray-200 px-5 py-4 bg-gray-50/50">
          <div className="flex items-center gap-3">
            <div className="flex-1 relative">
              <div className="bg-white border-2 border-dashed border-[#FF8C00]/30 rounded-xl px-4 py-3 text-center">
                <code className="font-display font-black text-xl text-[#FF8C00] tracking-widest select-all" data-testid={`coupon-code-${coupon.id}`}>
                  {coupon.code}
                </code>
              </div>
            </div>
          ...
          <button
            ...
            className="mt-3 w-full text-center text-sm text-[#228B22] hover:text-[#1a6b1a] font-semibold flex items-center justify-center gap-1 transition-colors"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full mb-4 border border-orange-100">
              <Ticket className="w-5 h-5 text-[#FF8C00]" />
            ...
            <form onSubmit={handleSearch} className="flex-1 max-w-md">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-100 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-[#FF8C00] outline-none"
            ...
            <div className="flex gap-2 overflow-x-auto hide-scrollbar">
              <button
                ...
                className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all ${
                  !selectedCategory ? 'bg-[#FF8C00] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all ${
                    selectedCategory === cat.name ? 'bg-[#FF8C00] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
